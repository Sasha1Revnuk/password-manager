...
/***/ "./resources/js/acc/panel/abonements.js":
/*!**********************************************!*\
  !*** ./resources/js/acc/panel/abonements.js ***!
  \**********************************************/
/*! no exports provided */
...
$(document).ready(function () {
  var table = $('#abonements').DataTable({
    processing: true,
    serverSide: true,
    lengthMenu: [10, 25, 50],
    searching: false,
    ...
    ajax: {
      url: '/account/api/object/' + $('#abonements').attr('data-object') + '/control-panel/abonements',
      type: "GET",
      data: {
        withCanceled: function withCanceled() {
          return $('#canceled').is(':checked') ? 1 : 0;
        },
        sections: function sections() {
          return $('#section-filter-btn').attr('data-sections');
        },
        visitorName: function visitorName() {
  ...
  $('#visitorName').on('change', function () {
    table.ajax.reload();
  });
  ...
  $('body').on('click', '.cancel-abonement',
  /*#__PURE__*/
  _asyncToGenerator(
  /*#__PURE__*/
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.next = 2;
            return Swal.fire({
              title: 'Причина відміни абонементу?',
              input: 'textarea',
              confirmButtonColor: '#dc3545',
              cancelButtonText: 'Не відміняти абонемент',
              inputPlaceholder: 'Причина відміни',
              inputAttributes: {
                'aria-label': 'Причина відміни'
              },
              showCancelButton: true,
              inputValidator: function inputValidator(value) {
                if (!value) {
                  return 'Ви не ввели причину відміни!';
                }
              }
            });
          ...
          case 2:
            _ref2 = _context.sent;
            description = _ref2.value;

            if (description) {
              Swal.fire({
                title: 'Ви впевнені?',
                text: 'Ви збираєтесь відмінити абонемент',
                type: 'question',
                showCancelButton: true,
                confirmButtonColor: '#dc3545',
                ...
                if (result.value) {
                  $.ajax({
                    type: 'GET',
                    url: '/account/api/object/' + $(_this).attr('data-object') + '/control-panel/abonements/change-status/' + $(_this).attr('id'),
                    headers: authHeaders,
                    data: {
  ...
  $('body').on('click', '.restore-abonement', function () {
    var _this2 = this;

    Swal.fire({
      title: 'Ви впевнені?',
      text: 'Ви збираєтесь відновити абонемент',
      type: 'question',
      showCancelButton: true,
      ...
      if (result.value) {
        $.ajax({
          type: 'GET',
          url: '/account/api/object/' + $(_this2).attr('data-object') + '/control-panel/abonements/restore/' + $(_this2).attr('id'),
          headers: authHeaders
        }).then(function (response) {
          table.ajax.reload();

          if (response === true) {
            Swal.fire("\u0410\u0431\u043E\u043D\u0435\u043C\u0435\u043D\u0442 \u0432\u0456\u0434\u043D\u043E\u0432\u043B\u0435\u043D\u043E");
          }
        });
      }
    });
  });
  $('body').on('click', '.add-visiting',
  /*#__PURE__*/
  _asyncToGenerator(
  /*#__PURE__*/
        ...
        switch (_context2.prev = _context2.next) {
          case 0:
            ...
            return Swal.fire({
              ...
              input: 'textarea',
              cancelButtonText: 'Не додавати відвідування',
              inputPlaceholder: 'Додаткові відомості (не обов\'язково)',
              inputAttributes: {
                'aria-label': 'Додаткові відомості (не обов\'язково)'
              },
              showCancelButton: true,
              preConfirm: function preConfirm(desc) {
                return $.ajax({
                  type: 'GET',
                  url: '/account/api/object/' + $(_this3).attr('data-object') + '/control-panel/abonements/add-visiting/' + $(_this3).attr('id'),
                  headers: authHeaders,
                  data: {
                    description: desc
                  }
  ...
  $('body').on('change', '#filter-sections', function () {
    if ($('#filter-sections').val()) {
      if ($('#filter-sections').val() != 0) {
        ...
        $.ajax({
          type: 'GET',
          url: '/account/api/object/' + $('#programSales').attr('data-object') + '/control-panel/program-categories/for-program/' + $('#filter-sections').val(),
          headers: authHeaders
        }).then(function (response) {
          $('#categories').empty();
...
/***/ 26:
/*!****************************************************!*\
  !*** multi ./resources/js/acc/panel/abonements.js ***!
  \****************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! D:\WEB\gym-control\resources\js\acc\panel\abonements.js */"./resources/js/acc/panel/abonements.js");
